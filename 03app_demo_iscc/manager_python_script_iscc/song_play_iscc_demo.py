#adjustment from original BroadcastLeds.py in smartmesh sdk-master https://github.com/dustcloud/smartmeshsdk
#put this file in the smartmesh sdk repo
#then execute it and connect to the embedded manager using connect COMXX where XX is the number of the 4th port of the manager.
#then type help to see the commands
...
PLAY_MODE_STOP         = 0
PLAY_MODE_STAR_WARS    = 1
PLAY_MODE_HARRY_POTTER = 2

SLOT_DURATION_S        = 0.00725
RESPONSE_TIME_S        = 5

play_mode              = PLAY_MODE_STOP
current_asn            = 0
...
class OapTxRx(object):
    ...
    def setAllLeds(self,ledVal): 
        ...
        current_asn = connector.dn_getTime().asn
    ...
    def _sendOap(self,macToSendTo):
        ...
        with self.dataLock:
            ...
            #current_asn      = connector.dn_getTime().asn
            current_asn_format = (current_asn[0]<<32) + (current_asn[1]<<24) + (current_asn[2]<<16) + (current_asn[3]<<8) + (current_asn[4]<<0)
            trigger_song_asn = int(current_asn_format + RESPONSE_TIME_S/(SLOT_DURATION_S))
            
            trigger_song_asn_format = [(trigger_song_asn>>32) & 0xFF, (trigger_song_asn>>24) & 0xFF, (trigger_song_asn>>16) & 0xFF, (trigger_song_asn>>8) & 0xFF, (trigger_song_asn>>0) & 0xFF]
            print("current_asn {} trigger_song_asn {}".format(current_asn, trigger_song_asn_format))
            ...
            connector.dn_sendData(
                ...
                options      = 0x00,
                data         = [OapTxRx.play_mode] + trigger_song_asn_format, #oap_msg
            )
...
def starwars_clicb(params):
    global oapTxRx
    OapTxRx.play_mode = PLAY_MODE_STAR_WARS
    oapTxRx.setAllLeds(1)

def harrypotter_clicb(params):
    global oapTxRx
    OapTxRx.play_mode = PLAY_MODE_HARRY_POTTER
    oapTxRx.setAllLeds(1)
...
def main():
    ...
    cli.registerCommand(
        name                      = 'starwars',
        alias                     = 'sw',
        description               = 'play star wars theme once',
        params                    = [],
        callback                  = starwars_clicb,
        dontCheckParamsLength     = False,
    )
    cli.registerCommand(
        name                      = 'harrypotter',
        alias                     = 'hp',
        description               = 'play harry potter theme once',
        params                    = [],
        callback                  = harrypotter_clicb,
        dontCheckParamsLength     = False,
    )
